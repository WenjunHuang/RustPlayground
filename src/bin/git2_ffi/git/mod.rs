...
fn check(code: c_int) -> Result<c_int> {
    ...
    unsafe {
        ...
        // libgit2 ensures thate (*error).message is always non-null and null
        // terminated, so this call is safe
        let message = CStr::from_ptr((*error).message).to_string_lossy()
                                                      .into_owned();
        Err(Error { code: code as i32,
                    message,
...
impl Repository {
    ...
    pub fn reference_name_to_id(&self, name: &str) -> Result<Oid> {
        ...
        unsafe {
            ...
            check(raw::git_reference_name_to_id(&mut oid,
                                                self.raw,
