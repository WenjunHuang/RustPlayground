  ...
  translate_button.connect_clicked(
    clone!(language_src,language_dst, translate_input => move |_| {
        ...
        let fut = translate(&string,&src, &dst, &client).and_then(|result|{
              ...
              glib::idle_add(print_translation_result);
              ok(())
        });
        core.run(fut).unwrap();
    }),
...
#[inline]
fn call_baidu_translate_api(url: hyper::Uri, client: &Client<HttpConnector>) -> impl Future<Item=hyper::Response, Error=Error> {
  client.get(url).from_err()
}

#[inline]
fn read_all_response_body(response: hyper::Response) -> impl Future<Item=hyper::Chunk, Error=Error> {
  response.body().concat2().from_err()
}

fn deserialize_json_result(bytes: &[u8]) -> impl Future<Item=APISuccess, Error=Error> {
  let v = from_slice::<Value>(bytes);
  match v {
    Ok(Value::Object(m)) => {
...
fn translate<'a>(
  ...
  generate_url(input, src, dst)
    .and_then(move |url| call_baidu_translate_api(url, client))// client.get(url).from_err())
    .and_then(|res| read_all_response_body(res))//.map_err(|err| Error::HyperError(err)))
    .and_then(move |body: Chunk| deserialize_json_result(&body))
}

fn print_translation_result() -> glib::Continue {
  GLOBAL.with(|global| {
