...
mod tutorial5 {
    ...
    pub fn create_ui(playbin: &gst::Element) {
        ...
        video_window.connect_realize(move |video_window| {
            ...
            if cfg!(feature = "tutorial5-x11") {
            ...
            } else if cfg!(feature="tutorial5-win32") {
                if display_type_name == "GdkWin32Display" {
                    ...
                    unsafe {
                        let window = gdk_win32_window_get_handle(gdk_window.to_glib_none().0);
                        video_overlay.set_window_handle(window as usize);
                    }
                } else {
                    println!(
    ...
    pub fn run() {
        ...
        if !cfg!(feature = "tutorial5-x11") && !cfg!(feature = "tutorial5-quartz") && !cfg!(feature="tutorial5-win32"){
            eprintln!("No Gdk backend selected, compile with --features tutorial5[-x11][-quartz][-win32].");
...
fn main() {
    println!("Please compile with --features tutorial5[-x11][-quartz][-win32]");
}